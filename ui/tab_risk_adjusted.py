...
class RiskAdjustedTab:
    ...
    def render(metrics: Dict[str, Any]) -> None:
        ...
        metric_ids_3y = [_METRIC_ID.get((k, 3)) for k in ratio_keys]
        metric_ids_5y = [_METRIC_ID.get((k, 5)) for k in ratio_keys]
        fig = go.Figure()
        fig.add_trace(go.Bar(name='3Y', x=ratio_names, y=vals_3y,
                             text=[f"{v:.2f}" for v in vals_3y], textposition='auto',
                             marker_color=[get_color(mid, v) for mid, v in zip(metric_ids_3y, vals_3y)]))
        fig.add_trace(go.Bar(name='5Y', x=ratio_names, y=vals_5y,
                             text=[f"{v:.2f}" for v in vals_5y], textposition='auto',
                             marker_color=[get_color(mid, v) for mid, v in zip(metric_ids_5y, vals_5y)]))
    ...
    @staticmethod
    def _ratio_bar(metrics, key, heading, title, yaxis_title):
        st.markdown(f"#### {heading}")
        data = metrics.get(key, {})
        vals = {y: data.get(y) for y in [3, 5] if not isinstance(data.get(y), dict) and data.get(y) is not None}
        if vals:
            colors = [get_color(_METRIC_ID.get((key, y)), v) for y, v in vals.items()]
