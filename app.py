...
if 'all_results' in st.session_state:
    ...
    def _style_table(df: pd.DataFrame):
        styled = df.set_index('Scheme Name').style \
            .format("{:.2f}") \
            .background_gradient(cmap='RdYlGn', subset=['Overall\nScore'], low=0.4, high=0.2) \
            .set_properties(**{'border': '1px solid #e6e9ef'}) \
            .applymap(lambda v: 'background-color: #ff4b4b; color: white;' if isinstance(v, (int, float)) and v == 0.0 else '') \
            .set_table_styles([
                # Bold the header row (category names)
                {'selector': 'thead th', 'props': [('font-weight', 'bold')]},
                # Bold the index column (scheme names)
                {'selector': 'tbody th', 'props': [('font-weight', 'bold')]},
            ])
        return styled

    st.table(_style_table(comparison_df))
